...
def charts(request):
    ...
    if albums_with_rating.exists():
        top_album = albums_with_rating.order_by('-average_rating', '-review_count').first()
    else:
        top_album = None

    # Fetch the next top 5 albums based on average rating, excluding the top album
    top_albums = (albums_with_rating.exclude(pk=top_album.pk)
                .order_by('-average_rating')[:5] if top_album else [])
